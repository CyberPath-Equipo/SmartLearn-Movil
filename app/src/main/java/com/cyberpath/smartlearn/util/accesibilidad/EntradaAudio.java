...
public class EntradaAudio implements RecognitionListener {
    ...
    public void confirmarAfirmacion(OnConfirmacionListener listener) {
        ...
        // pedir confirmación sin TTS aquí (el orchestrador debe hablar y luego llamar a iniciarEscucha),
        // pero para compatibilidad: si quieres que EntradaAudio hable el prompt, hazlo mediante SalidaAudio y callback.
        iniciarEscucha();
    }
    ...
    private void iniciarEscucha() {
        ...
        // Siempre iniciar en main thread
        mainHandler.post(() -> {
            try {
                estaEscuchando = true;
    ...
    public void detenerEscucha() {
        if (speechRecognizer != null && estaEscuchando) {
            try {
                // stopListening también debe ejecutarse en main
                mainHandler.post(() -> {
                    try {
    ...
    public void onResults(Bundle results) {
        ...
        // No coincidió: pedir reintento simple sin TTS para no bloquear (o avisar si quieres)
        if (salidaAudio != null) {
